...
class TestMidiItemClientOperations(unittest.TestCase):
    ...
    def test_midi_and_item_operations(self):
        ...
        midi_item_id = self.controller.create_midi_item(midi_track_index, MIDIController.DEFAULT_MIDI_START_TIME, length=MIDIController.DEFAULT_MIDI_LENGTH)
        ...
        for pitch in MIDIController.DEFAULT_MIDI_NOTE_PITCHES:
            self.assertTrue(self.controller.add_midi_note(midi_track_index, midi_item_id, MIDIController.MIDINoteParams(pitch=pitch, start_time=0.0, length=MIDIController.DEFAULT_MIDI_NOTE_LENGTH, velocity=MIDIController.DEFAULT_MIDI_NOTE_VELOCITY)), f"Failed to add note with pitch {pitch}.")
        self.logger.info("Added MIDI notes (C major chord)")

        midi_item2_id = self.controller.create_midi_item(midi_track_index, MIDIController.DEFAULT_SECOND_MIDI_START_TIME, length=MIDIController.DEFAULT_SECOND_MIDI_LENGTH)
        ...
        self.assertTrue(self.controller.add_midi_note(midi_track_index, midi_item2_id, MIDIController.MIDINoteParams(pitch=MIDIController.DEFAULT_SECOND_MIDI_NOTE_PITCH, start_time=0.0, length=MIDIController.DEFAULT_SECOND_MIDI_NOTE_LEN, velocity=MIDIController.DEFAULT_SECOND_MIDI_NOTE_VEL)), "Failed to add note to second item.")
