...
class TestReaperController(unittest.TestCase):
    ...
    def _test_parameter_modification(
        ...
        if not parameters_modified:
            self.logger.warning(
                "Could not verify parameter value changes, but set operations were successful."
            )
    ...
    def _test_single_parameter(
        self, track_index: int, fx_index: int, param: dict
    ) -> bool:
        ...
        new_value = (
            ...
            else current_value * DEFAULT_PARAMETER_MULTIPLIER
        )
    ...
    def _test_mp3_insertion(self, track_index: int):
        ...
        self.assertTrue(
            os.path.exists(self.sample_audio_path), "Test MP3 file should exist"
        )
    ...
    def _test_mp3_properties(self, track_index: int, mp3_item_id):
        ...
        if not self.controller.debug:
            self.assertIn(
                "position", mp3_properties, "MP3 item should have a position property"
            )
            self.assertIn(
                "length", mp3_properties, "MP3 item should have a length property"
            )
            if "length" in mp3_properties:
                self.assertGreater(
                    mp3_properties["length"],
                    0,
                    "MP3 item length should be greater than zero",
                )
        ...
        if not self.controller.debug:
            ...
            if "position" in updated_properties:
                self.assertAlmostEqual(
                    updated_properties["position"],
                    DEFAULT_MP3_NEW_POSITION,
                    delta=DEFAULT_POSITION_DELTA,
                )
            if "length" in updated_properties:
                self.assertAlmostEqual(
                    updated_properties["length"],
                    DEFAULT_MP3_NEW_LENGTH,
                    delta=DEFAULT_LENGTH_DELTA,
                )
    ...
    def _test_mp3_item_operations(self, track_index: int):
        ...
        if not self.controller.debug:
            ...
            if "position" in updated_mp3_properties:
                self.assertAlmostEqual(
                    updated_mp3_properties["position"],
                    new_mp3_position,
                    delta=DEFAULT_POSITION_DELTA,
    ...
    def test_midi_item_create_fail(self):
        ...
        if not self.controller.debug:
            self.assertEqual(
                midi_item_id, -1, "MIDI item creation should fail and return -1"
            )
        else:
            # In debug mode, it may return -1 or a valid ID, so just check for int type
            self.assertIsInstance(
                midi_item_id, int, "MIDI item ID should be int in debug mode"
            )
