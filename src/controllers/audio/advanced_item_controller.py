...
class AdvancedItemController:
    ...
    def split_item(
        self, track_index: int, item_index: int, split_time: float
    ) -> List[int]:
        """
        ...
        try:
            ...
            for i, new_item in enumerate(track.items):
                if (
                    new_item.position >= split_time
                    or new_item.position + new_item.length > split_time
                ):
                    new_items.append(i)

            self.logger.info(
                f"Split item {item_index} at {split_time}s, created {len(new_items)} new items"
            )
            return new_items
    ...
    def fade_in(
        self, track_index: int, item_index: int, fade_length: float, fade_curve: int = 0
    ) -> bool:
        """
        ...
        try:
            ...
            self._RPR.SetMediaItemInfo_Value(
                item.id, "D_FADEOUTLEN", 0
            )  # Clear fade-out first
            ...
            self.logger.info(
                f"Added {fade_length}s fade-in to item {item_index} on track {track_index}"
            )
            return True
    ...
    def fade_out(
        ...
        try:
            ...
            self.logger.info(
                f"Added {fade_length}s fade-out to item {item_index} on track {track_index}"
            )
            return True
    ...
    def crossfade_items(
        ...
        try:
            ...
            self.logger.info(
                f"Created crossfade between items {item1_index} and {item2_index}"
            )
            return True
