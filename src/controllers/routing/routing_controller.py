...
class RoutingController:
    ...
    def add_send(
        ...
        try:
            ...
            if source is None or destination is None:
                self.logger.error(
                    f"Failed to get tracks: source={source_track}, destination={destination_track}"
                )
                return None

            self.logger.info(
                f"Attempting to add send from track {source_track} to track {destination_track}"
            )
            self.logger.info(
                f"Source track: {source.name}, Destination track: {destination.name}"
            )
            ...
            if send_id < 0:
                self.logger.error(
                    f"Failed to create send using ReaScript API: {send_id}"
                )
                return None
            ...
            RPR.SetTrackSendInfo_Value(
                source.id, 0, send_id, "B_MUTE", 1 if mute else 0
            )
            RPR.SetTrackSendInfo_Value(
                source.id, 0, send_id, "B_PHASE", 1 if phase else 0
            )
            ...
            self.logger.info(
                f"After creation: Track {source_track} now has {verify_count} sends"
            )

            # Try to read back the send we just created
            if send_id >= 0 and send_id < verify_count:
                verify_dest = RPR.GetTrackSendInfo_Value(
                    source.id, 0, send_id, "P_DESTTRACK"
                )
                verify_vol = RPR.GetTrackSendInfo_Value(source.id, 0, send_id, "D_VOL")
                self.logger.info(
                    f"Verification - Send {send_id}: dest_ptr={verify_dest}, volume={verify_vol}"
                )
    ...
    def remove_send(self, source_track: int, send_id: int) -> bool:
        ...
        try:
            ...
            result = RPR.RemoveTrackSend(
                source.id, 0, send_id
            )  # 0 for sends, 1 for receives
    ...
    def get_sends(self, track_index: int) -> List[SendInfo]:
        ...
        try:
            ...
            sends = []
            send_count = RPR.GetTrackNumSends(
                track.id, 0
            )  # 0 for sends, 1 for receives

            self.logger.info(
                f"Track {track_index} (ID: {track.id}): Found {send_count} sends"
            )

            for i in range(send_count):
                try:
                    # Get destination track pointer
                    dest_track_pointer = RPR.GetTrackSendInfo_Value(
                        track.id, 0, i, "P_DESTTRACK"
                    )
                    ...
                    if isinstance(
                        dest_track_pointer, str
                    ) and dest_track_pointer.startswith("(MediaTrack*)0x"):
                    ...
                    if dest_track_ptr_int != 0:
                        ...
                        for idx, project_track in enumerate(project.tracks):
                            ...
                            if isinstance(
                                project_track.id, str
                            ) and project_track.id.startswith("(MediaTrack*)0x"):
                                hex_value = project_track.id.replace(
                                    "(MediaTrack*)0x", ""
                                )
                    ...
                    if dest_track_index == -1:
                        self.logger.debug(
                            f"Could not determine destination track index for send {i}, using -1"
                        )
                        dest_track_index = -1
                    ...
                    channels = int(
                        RPR.GetTrackSendInfo_Value(track.id, 0, i, "I_SRCCHAN")
                    )
                    ...
                    self.logger.debug(
                        f"Send {i}: {track_index} -> {dest_track_index}, vol={volume}, pan={pan}, mute={mute}"
                    )
            ...
            self.logger.info(
                f"Successfully processed {len(sends)} sends on track {track_index}"
            )
            return sends
    ...
    def get_receives(self, track_index: int) -> List[ReceiveInfo]:
        ...
        try:
            ...
            self.logger.info(
                f"Scanning all tracks for sends to track {track_index} (target track ID: {target_track.id})"
            )

            # Convert target track ID to int for comparison
            target_track_id_int = 0
            if isinstance(target_track.id, str) and target_track.id.startswith(
                "(MediaTrack*)0x"
            ):
                ...
                try:
                    ...
                    for send_idx in range(send_count):
                        try:
                            ...
                            dest_track_pointer = RPR.GetTrackSendInfo_Value(
                                source_track.id, 0, send_idx, "P_DESTTRACK"
                            )
                            ...
                            if isinstance(
                                ...
                                hex_value = dest_track_pointer.replace(
                                    "(MediaTrack*)0x", ""
                                )
                                dest_track_ptr_int = int(hex_value, 16)
                            elif isinstance(dest_track_pointer, (int, float)):
                            ...
                            if dest_track_ptr_int == target_track_id_int:
                                ...
                                volume = RPR.GetTrackSendInfo_Value(
                                    source_track.id, 0, send_idx, "D_VOL"
                                )
                                pan = RPR.GetTrackSendInfo_Value(
                                    source_track.id, 0, send_idx, "D_PAN"
                                )
                                mute = bool(
                                    RPR.GetTrackSendInfo_Value(
                                        source_track.id, 0, send_idx, "B_MUTE"
                                    )
                                )
                                phase = bool(
                                    RPR.GetTrackSendInfo_Value(
                                        source_track.id, 0, send_idx, "B_PHASE"
                                    )
                                )
                                channels = int(
                                    RPR.GetTrackSendInfo_Value(
                                        source_track.id, 0, send_idx, "I_SRCCHAN"
                                    )
                                )
                                ...
                                self.logger.debug(
                                    f"Found receive: Track {source_idx} -> Track {track_index}, vol={volume}, pan={pan}, mute={mute}"
                                )

                        except Exception as send_error:
                            self.logger.debug(
                                f"Error checking send {send_idx} on track {source_idx}: {send_error}"
                            )

                except Exception as track_error:
                    self.logger.debug(
                        f"Error scanning track {source_idx}: {track_error}"
                    )
    ...
    def toggle_send_mute(
        self, source_track: int, send_id: int, mute: Optional[bool] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if mute is None:
                ...
                current_mute = RPR.GetTrackSendInfo_Value(
                    source.id, 0, send_id, "B_MUTE"
                )
                mute = not bool(current_mute)

            result = RPR.SetTrackSendInfo_Value(
                source.id, 0, send_id, "B_MUTE", 1 if mute else 0
            )
    ...
    def clear_all_receives(self, track_index: int) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Cleared {receive_count} receives from track {track_index}"
            )
            return True
