...
class AdvancedRoutingController:
    ...
    def create_folder_track(self, name: str = "Folder Track") -> int:
        ...
        try:
            ...
            self._RPR.InsertTrackAtIndex(
                track_index, True
            )  # Use InsertTrackAtIndex instead of InsertMediaTrack
    ...
    def set_track_parent(self, child_track_index: int, parent_track_index: int) -> bool:
        ...
        try:
            ...
            if child_track_index >= len(project.tracks) or parent_track_index >= len(
                project.tracks
            ):
    ...
    def get_track_children(self, parent_track_index: int) -> List[int]:
        ...
        try:
            ...
            children = []
            parent_depth = self._RPR.GetMediaTrackInfo_Value(
                project.tracks[parent_track_index].id, "I_FOLDERDEPTH"
            )
            ...
            for i in range(parent_track_index + 1, len(project.tracks)):
                ...
                track_depth = self._RPR.GetMediaTrackInfo_Value(
                    track.id, "I_FOLDERDEPTH"
                )
            ...
            self.logger.info(
                f"Found {len(children)} children for track {parent_track_index}"
            )
            return children
